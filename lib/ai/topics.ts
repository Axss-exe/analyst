...
Only output valid JSON.`;
  ...
  const response = await generateWithAI(prompt, {
    systemPrompt:
      "You are a thematic analysis engine. Extract structured topics and themes from intelligence text.",
    temperature: 0.1,
    maxTokens: 1024,
  });
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      return {
        ...
        geographicFocus: Array.isArray(parsed.geographicFocus)
