...
export async function extractEntitiesFromText(
  ...
  console.log(
    `[entities] Processing sample of ${estimateTokens(sample)} tokens`,
  );
...
- Only output valid JSON array. No markdown, no explanations.`;
  ...
  const response = await generateWithAI(prompt, {
    systemPrompt:
      "You are an entity extraction system. Extract only entities explicitly mentioned in the text. NEVER return empty names.",
    temperature: 0.1,
    maxTokens: 2048,
  });
  ...
  if (parsed && Array.isArray(parsed)) {
    ...
    if (valid.length !== parsed.length) {
      console.warn(
        `[entities] Filtered ${parsed.length - valid.length} invalid entities from LLM response`,
      );
    }
  ...
  console.warn(
    "[entities] Failed to parse LLM response:",
...
> {
  ...
  const prompt = `Extract timeline events with dates from the following text. For each event, identify which entities (people, organizations, locations, projects) are involved.
...
Respond in this JSON format only:
[
  { "date": "YYYY-MM-DD", "title": "Event Title", "description": "Brief description", "entityNames": ["Entity Name 1", "Entity Name 2"] }
]

CRITICAL RULES:
- Every event MUST have non-empty "date", "title", "description", and "entityNames".
- "entityNames" must list the names of entities explicitly involved in this event.
...
- Only output valid JSON array. No markdown, no explanations.`;
  ...
  const response = await generateWithAI(prompt, {
    systemPrompt:
      "You are a timeline extraction system. Extract only events with explicit or strongly implied dates, and identify the entities involved in each.",
    temperature: 0.1,
    maxTokens: 2048,
  });
  ...
  if (parsed && Array.isArray(parsed)) {
    ...
    if (valid.length !== parsed.length) {
      console.warn(
        `[timeline] Filtered ${parsed.length - valid.length} invalid events from LLM response`,
      );
    }
    return valid.map((e: any) => ({
      date: e.date.trim(),
      ...
      entityNames: Array.isArray(e.entityNames)
        ? e.entityNames.filter(
  ...
  console.warn(
    "[timeline] Failed to parse LLM response:",
    response.slice(0, 200),
  );
  return [];
}
