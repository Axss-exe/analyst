...
async function processQueue() {
  ...
  while (state.queue.length > 0) {
    ...
    if (state.requestsThisMinute >= MAX_RPM) {
      ...
      console.log(`[rate-limiter] RPM limit (${MAX_RPM}) reached. Waiting ${wait}ms for window reset...`)
      await sleep(wait)
      resetWindowIfNeeded()
    }
...
export async function acquireSlot(tokens: number): Promise<void> {
  return new Promise((resolve) => {
    state.queue.push(() => {
      ...
      console.log(`[rate-limiter] Slot acquired. RPM: ${state.requestsThisMinute}/${MAX_RPM}, TPM: ${state.tokensThisMinute}/${MAX_TPM}, Tokens: ${tokens}`)
      resolve()
    })
    processQueue()
  })
