...
function evaluate()[] {
  ...
  // ─── FACET 1: Summaries ───
  {
    ...
    const evidenceRows = db.prepare("SELECT id, title, summary FROM evidence ORDER BY id").all() ;
    const withSummary = evidenceRows.filter((e) => e.summary && e.summary.length > 100);
  ...
  // ─── FACET 3: Facts ───
  {
    ...
    const orphaned = facts.filter((f) => !f.evidence_id);
  ...
  // ─── FACET 4: v4 Intelligence ───
  {
    ...
    const counts = {
      ...
      outcomes: (db.prepare("SELECT COUNT(*) as c FROM outcomes").get() ).c,
  ...
  // ─── FACET 5: Timeline ───
  {
    ...
    const uniqueEvidence = new Set(events.map((e) => e.evidence_id).filter(Boolean));
    ...
    const dates = events.map((e) => e.date).join(" ").toLowerCase();
  ...
  // ─── FACET 6: Story Candidates ───
  {
    ...
    const candidates = db.prepare("SELECT * FROM story_candidates WHERE status != 'rejected'").all() ;
  ...
  // ─── FACET 7: Narratives ───
  {
    ...
    const narratives = db.prepare("SELECT * FROM narratives WHERE generation_type = 'auto'").all() ;
  ...
  // ─── FACET 8: Graph ───
  {
    ...
    const rels = db.prepare("SELECT * FROM story_relationships").all() ;
    if (rels.length >= 4) {
      score += 3;
    ...
    const storyEdges = db.prepare("SELECT * FROM story_graph_edges").all() ;
    const contextEdges = db.prepare("SELECT * FROM context_graph_edges").all() ;
  ...
  // ─── FACET 9: Tasks ───
  {
    ...
    const tasks = db.prepare("SELECT * FROM research_tasks").all() ;
    if (tasks.length > 0) {
      score += 2;
    ...
    const taskEvidence = db.prepare("SELECT * FROM task_evidence").all() ;
  ...
  // ─── FACET 10: Briefs ───
  {
    ...
    const briefs = db.prepare("SELECT * FROM generated_briefs").all() ;
    if (briefs.length > 0) {
      score += 2;
