...
export const evidence = sqliteTable("evidence", {
  ...
  collectionDate: text("collection_date").notNull().default(sql`(datetime('now'))`),
  ...
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("evidence_created_by_idx").on(t.createdBy), index("evidence_tags_idx").on(t.tags)])

export const stories = sqliteTable("stories", {
  id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  title: text("title").notNull(),
  overview: text("overview").notNull(),
  status: text("status", { enum: ["active", "archived", "closed"] }).notNull().default("active"),
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
  updatedAt: text("updated_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("stories_status_idx").on(t.status), index("stories_created_by_idx").on(t.createdBy)])

export const storyEvidence = sqliteTable("story_evidence", {
  id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  ...
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [uniqueIndex("story_evidence_unique_idx").on(t.storyId, t.evidenceId), index("story_evidence_story_idx").on(t.storyId), index("story_evidence_evidence_idx").on(t.evidenceId)])
...
export const relationships = sqliteTable("relationships", {
  ...
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("rel_source_idx").on(t.sourceId), index("rel_target_idx").on(t.targetId), index("rel_type_idx").on(t.type)])
...
export const timelineEvents = sqliteTable("timeline_events", {
  ...
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("timeline_date_idx").on(t.date), index("timeline_story_idx").on(t.storyId), index("timeline_evidence_idx").on(t.evidenceId)])
...
export const researchTasks = sqliteTable("research_tasks", {
  ...
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("tasks_owner_idx").on(t.ownerId), index("tasks_status_idx").on(t.status), index("tasks_priority_idx").on(t.priority)])

export const generatedBriefs = sqliteTable("generated_briefs", {
  id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  storyId: integer("story_id").notNull().references(() => stories.id, { onDelete: "cascade" }),
  headline: text("headline").notNull(),
  content: text("content").notNull(),
  version: integer("version").notNull().default(1),
  generationMode: text("generation_mode", { enum: ["full", "partial", "since_last"] }).notNull(),
  evidenceIds: text("evidence_ids").notNull().default("[]"),
  ...
  createdBy: integer("created_by").notNull().references(() => users.id),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("briefs_story_idx").on(t.storyId), index("briefs_created_idx").on(t.createdAt)])
...
export const auditLog = sqliteTable("audit_log", {
  ...
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [index("audit_user_idx").on(t.userId), index("audit_action_idx").on(t.action), index("audit_target_idx").on(t.targetType, t.targetId)])
...
export const evidenceEntities = sqliteTable("evidence_entities", {
  id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  evidenceId: integer("evidence_id").notNull().references(() => evidence.id, { onDelete: "cascade" }),
  entityId: integer("entity_id").notNull().references(() => entities.id, { onDelete: "cascade" }),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
}, (t) => [uniqueIndex("evidence_entity_unique_idx").on(t.evidenceId, t.entityId)])

export const taskEvidence = sqliteTable("task_evidence", {
  id: integer("id", { mode: "number" }).primaryKey({ autoIncrement: true }),
  taskId: integer("task_id").notNull().references(() => researchTasks.id, { onDelete: "cascade" }),
  evidenceId: integer("evidence_id").notNull().references(() => evidence.id, { onDelete: "cascade" }),
  createdAt: text("created_at").notNull().default(sql`(datetime('now'))`),
