      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {[
            ...
            { label: "Evidence", value: stats?.totalEvidence || 0, icon: FileText },
            { label: "Stories", value: stats?.totalStories || 0, icon: BookOpen },
            { label: "Tasks", value: stats?.totalTasks || 0, icon: ClipboardList },
            { label: "Briefs", value: stats?.totalBriefs || 0, icon: Newspaper },
            { label: "Entities", value: stats?.totalEntities || 0, icon: Activity },
          ].map((s) => (
        ...
        <Tabs defaultValue="users">
          <TabsList>
            ...
            <TabsTrigger value="audit"><Activity className="mr-1 h-3 w-3" /> Audit Log</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="users" className="mt-4">
            <div className="rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow><TableHead>Name</TableHead><TableHead>Email</TableHead><TableHead>Role</TableHead><TableHead>Status</TableHead><TableHead>Created</TableHead><TableHead className="w-[120px]">Actions</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {users.map((u) => (
                    <TableRow key={u.id}>
                      ...
                      <TableCell>{u.isBlocked ? <Badge variant="destructive">Blocked</Badge> : <Badge variant="default">Active</Badge>}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{new Date(u.createdAt).toLocaleDateString()}</TableCell>
                      <TableCell>
                        {u.isBlocked ? (
                          <Button size="sm" variant="outline" onClick={() => handleUnblock(u.id)}><CheckCircle className="mr-1 h-3 w-3" /> Unblock</Button>
                        ) : (
                          <Button size="sm" variant="destructive" onClick={() => handleBlock(u.id)}><Ban className="mr-1 h-3 w-3" /> Block</Button>
                        )}
                      </TableCell>
                    </TableRow>
                  ))}
          ...
          <TabsContent value="audit" className="mt-4">
            <div className="rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow><TableHead>Action</TableHead><TableHead>Target</TableHead><TableHead>Actor</TableHead><TableHead>Time</TableHead></TableRow>
                </TableHeader>
                <TableBody>
                  {logs.map((log) => (
                    <TableRow key={log.id}>
