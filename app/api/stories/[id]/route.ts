...
export async function GET(
  ...
  try {
    ...
    const events = db
      .select()
      .from(timelineEvents)
      .where(eq(timelineEvents.storyId, id))
      .orderBy(timelineEvents.date)
      .all();
    const tasks = db
      .select()
      .from(researchTasks)
      .where(sql`${researchTasks.objective} LIKE ${"%story:" + id + "%"}`)
      .all();
    const briefs = db
      .select()
    ...
    const storyEntities =
      entityIdSet.size > 0
        ? db
            .select()
            .from(entities)
            .where(inArray(entities.id, Array.from(entityIdSet)))
            .all()
        : [];

    const entityIdArray = Array.from(entityIdSet);
    const rels =
      entityIdArray.length > 0
        ? db
            .select()
            .from(relationships)
            .where(
              sql`${relationships.sourceId} IN ${entityIdArray} OR ${relationships.targetId} IN ${entityIdArray}`,
            )
            .all()
        : [];
