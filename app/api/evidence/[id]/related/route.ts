...
export async function GET(
  ...
  try {
    ...
    if (targetStoryIds.length > 0) {
      ...
      if (relatedIds.length > 0) {
        storyRelated = db
          ...
          .from(evidence)
          .where(inArray(evidence.id, relatedIds))
          .limit(10)
          .all();
      }
    }
    ...
    if (targetEntityIds.length > 0) {
      const entityEvidenceLinks = db
        .select({ evidenceId: evidenceEntities.evidenceId })
        .from(evidenceEntities)
        .where(inArray(evidenceEntities.entityId, targetEntityIds))
        .all();
      const relatedIds = [
        ...new Set(entityEvidenceLinks.map((ee) => ee.evidenceId)),
      ].filter((id) => id !== evidenceId);
      if (relatedIds.length > 0) {
        entityRelated = db
          .select({
