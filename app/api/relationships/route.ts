...
export async function GET(request: NextRequest) {
  try {
    ...
    if (entityId) {
      ...
      query = query.where(or(eq(relationships.sourceId, eid), eq(relationships.targetId, eid))) as any
    }
    if (type) query = query.where(eq(relationships.type, type)) as any

    const items = query.orderBy(desc(relationships.createdAt)).limit(limit).all()
    const count = db.select({ count: sql<number>`count(*)` }).from(relationships).get()

    return NextResponse.json({ relationships: items, total: count?.count || 0 })
  } catch (error: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sourceId || !targetId || !type) {
      return NextResponse.json({ error: "Source, target, and type required" }, { status: 400 })
    }

    const source = db.select().from(entities).where(eq(entities.id, sourceId)).get()
    const target = db.select().from(entities).where(eq(entities.id, targetId)).get()
    if (!source || !target) {
