...
import { researchTasks, taskEvidence, taskEntities, evidence, entities } from "@/db/schema"
...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const task = db.select().from(researchTasks).where(eq(researchTasks.id, id)).get()
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = db.select().from(researchTasks).where(eq(researchTasks.id, id)).get()
    if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 })

    const body = await request.json()
    const { objective, priority, ownerId, deadline, status, completionNotes } = body
    ...
    if (status === "completed" && existing.status !== "completed") {
      await notifyTaskCompleted(id, existing.objective, existing.ownerId, user.id)
    }
