...
import { entities, evidenceEntities, evidence, relationships } from "@/db/schema"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const items = query.orderBy(desc(entities.createdAt)).limit(limit).offset(offset).all()
    const count = db.select({ count: sql<number>`count(*)` }).from(entities).get()
...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = db.select().from(entities).where(eq(entities.name, name)).get()
    if (existing) {
      return NextResponse.json({ error: "Entity with this name already exists", entity: existing }, { status: 409 })
    }
