...
export async function GET(request: NextRequest) {
  try {
    ...
    if (storyId)
      query = query.where(eq(timelineEvents.storyId, parseInt(storyId))) as any;
    if (evidenceId)
      query = query.where(
        eq(timelineEvents.evidenceId, parseInt(evidenceId)),
      ) as any;
    if (fromDate)
      query = query.where(gte(timelineEvents.date, fromDate)) as any;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!date || !title || !description) {
      return NextResponse.json(
        { error: "Date, title, and description required" },
        { status: 400 },
      );
    }
