...
const typeColors: Record<string, string> = {
  ...
  brief: "bg-rose-500/20 text-rose-400",
  timeline: "bg-cyan-500/20 text-cyan-400",
};
...
export default function SearchPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    const res = await fetch(
      `/api/search?q=${encodeURIComponent(query)}&type=${type}&limit=50`,
    );
    ...
    <AppShell>
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <form onSubmit={handleSearch} className="flex gap-2">
          <Input
            placeholder="Search evidence, stories, entities, tasks, briefs, timeline..."
            value={query}
          ...
          {results.map((result) => {
            ...
            return (
              <Card
                ...
                onClick={() => router.push(getUrl(result))}
              >
                <CardContent className="flex items-start gap-3 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        className={`text-[10px] capitalize ${typeColors[result.type] || "bg-muted"}`}
                      >
                        {result.type}
                      </Badge>
                    </div>
