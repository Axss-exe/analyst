  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/tasks", {
        ...
        body: JSON.stringify({ objective, priority, ownerId: parseInt(ownerId), deadline: deadline || null }),
      })
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <Card>
          <CardHeader><CardTitle>Create Research Task</CardTitle></CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2"><Label>Objective *</Label><Textarea value={objective} onChange={(e) => setObjective(e.target.value)} rows={3} required placeholder="What needs to be investigated?" /></div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button type="submit" disabled={loading}>{loading ? <><Loader2 className="mr-1 h-4 w-4 animate-spin" /> Saving...</> : "Create Task"}</Button>
              </div>
            </form>
          </CardContent>
        </Card>
