...
export default function EvidenceDetailPage() {
    ...
    return (
      <AppShell>
        <div className="flex h-96 flex-col items-center justify-center gap-4">
          <p className="text-muted-foreground">{error || "Evidence not found"}</p>
          <Link href="/evidence"><Button variant="outline">Back to Evidence</Button></Link>
        </div>
      </AppShell>
    )
  }
  ...
  const getRelationBadge = (types: string[]) => {
    if (types.includes("same_story")) return <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Same Story</Badge>
    if (types.includes("shared_entities")) return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">Shared Entities</Badge>
    if (types.includes("topic_overlap")) return <Badge className="bg-amber-500/20 text-amber-400 border-amber-500/30">Topic Match</Badge>
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            ...
            {ev.publicationDate && (
              <span className="text-xs text-muted-foreground">{new Date(ev.publicationDate).toLocaleDateString()}</span>
            )}
          </div>
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">{(ev.confidence * 100).toFixed(0)}%</span>
          </div>
          {meta.confidenceEvaluation && (
            <div className="mt-2 rounded-md bg-muted p-3 text-sm">
              <p className="font-medium">AI Confidence Assessment</p>
              <p className="text-muted-foreground text-xs mt-1">{meta.confidenceEvaluation.reasoning}</p>
              {meta.confidenceEvaluation.factors?.length > 0 && (
                <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
                  {meta.confidenceEvaluation.factors.map((f: string, i: number) => (
                    <li key={i}>• {f}</li>
                  ))}
                </ul>
              )}
          ...
          {meta.topics?.topics?.length > 0 && (
            <div className="mt-2 flex flex-wrap gap-1">
              {meta.topics.topics.map((t: string, i: number) => (
                <Badge key={i} variant="outline" className="text-[10px] bg-primary/5">{t}</Badge>
              ))}
            </div>
          )}
        </div>
        ...
        <Tabs defaultValue="summary">
          <TabsList>
            ...
            <TabsTrigger value="related">
              <GitBranch className="mr-1 h-3 w-3" /> Related Evidence ({data.relatedEvidence?.length || 0})
            </TabsTrigger>
              ...
              <CardContent className="py-4">
                ...
                {meta.summaryMethod === "fallback_too_large" && (
                  <p className="mt-3 text-xs text-amber-600 bg-amber-50 p-2 rounded">
                    This document was too large for automatic summarization. Please edit the summary manually.
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            {(!data.relatedEvidence || data.relatedEvidence.length === 0) ? (
              <Card>
                <CardContent className="py-8 text-center text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Related evidence appears when other items share entities, topics, or stories with this evidence.</p>
                </CardContent>
              </Card>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <Link href={`/evidence/${rel.id}`} className="text-sm font-medium hover:text-primary">
                              {rel.title}
                            </Link>
                            {getRelationBadge(rel.relationTypes)}
                          </div>
                          <p className="text-xs text-muted-foreground mt-1">{rel.source} | {(rel.relationScore * 100).toFixed(0)}% match</p>
                          ...
                          {rel.sharedEntityCount > 0 && (
                            <p className="text-[10px] text-muted-foreground mt-1">{rel.sharedEntityCount} shared entities</p>
                          )}
                        </div>
                      </div>
                    </CardContent>
            ...
            {data.linkedStories.length === 0 ? (
              <Card>
                <CardContent className="py-8 text-center text-muted-foreground">
                  ...
                  <Link href="/discover">
                    <Button variant="outline" size="sm" className="mt-3">Run Story Discovery</Button>
                  </Link>
                </CardContent>
              </Card>
            ) : (
          ...
          <TabsContent value="metadata" className="mt-4">
            <Card>
              <CardContent className="py-4">
                <pre className="text-xs bg-muted p-3 rounded-md overflow-auto">{JSON.stringify(meta, null, 2)}</pre>
              </CardContent>
            </Card>
          </TabsContent>
        </Tabs>
