...
export default function TemplatesPage() {
  ...
  const getConfig = (c: string) => { try { return JSON.parse(c) } catch { return {} } }
    ...
    <AppShell>
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Brief presentation templates</p>
        </div>
            ...
            {templates.map((t) => {
              ...
              return (
                <Card key={t.id} className="overflow-hidden">
                  <div className="h-2" style={{ backgroundColor: cfg.primaryColor || "#2563eb" }} />
                  <CardHeader className="pb-2">
                    <CardTitle className="text-base">{t.name}</CardTitle>
                    <Badge variant="outline" className="capitalize text-[10px] w-fit">{t.type}</Badge>
                  </CardHeader>
                  <CardContent>
