...
export default function TimelinePage() {
  ...
  const handleFilter = (e: React.FormEvent) => { e.preventDefault(); fetchEvents() }
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Chronological view of intelligence events</p>
          </div>
        </div>
                ...
                <Card>
                  <CardContent className="py-4">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm font-medium">{new Date(evt.date).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}</span>
                    </div>
                    ...
                    <div className="flex gap-2 mt-3">
                      {evt.evidenceId && (
                        <Link href={`/evidence/${evt.evidenceId}`}>
                          <Badge variant="outline" className="text-[10px] cursor-pointer hover:bg-accent">Evidence #{evt.evidenceId}</Badge>
                        </Link>
                      )}
                      {evt.storyId && (
                        <Link href={`/stories/${evt.storyId}`}>
                          <Badge variant="outline" className="text-[10px] cursor-pointer hover:bg-accent">Story #{evt.storyId}</Badge>
                        </Link>
                      )}
                    </div>
                  </CardContent>
